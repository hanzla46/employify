...
const SkillsForm = ({setSkills, setHasUploadedSkills}) => {
  const [activeTab, setActiveTab] = useState('hard');
  const [hardSkills, setHardSkills] = useState([
    { id: 1, name: '', experience: '', level: '' }
  ]);
  ...
  const [projects, setProjects] = useState([
    { id: 1, name: '', role: '', duration: '', technologies: '', description: '' }
  ]);
  ...
  const handleSubmit = (e) => {
    ...
    setHasUploadedSkills(true);
    setSkills([...hardSkills, ...softSkills, ...jobs, ...projects]);
    ...
    return hardSkills.map((skill) => (
      <div key={`hard-${skill.id}`} className="mb-6 p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label htmlFor={`hard-skill-level-${skill.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Skill Level
            </label>
            <select
              id={`hard-skill-level-${skill.id}`}
              value={skill.level}
              onChange={(e) => handleChange('hard', skill.id, 'level', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
              required
            >
    ...
    return softSkills.map((skill) => (
      <div key={`soft-${skill.id}`} className="mb-6 p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor={`soft-skill-proficiency-${skill.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Proficiency
            </label>
            <select
  ...
  const renderJobs = () => {
    return jobs.map((job) => (
      <div key={`job-${job.id}`} className="mb-6 p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800">
        ...
        <div>
          <label htmlFor={`job-description-${job.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Job Description
          </label>
          <textarea
            id={`job-description-${job.id}`}
            value={job.description}
            onChange={(e) => handleChange('job', job.id, 'description', e.target.value)}
            rows="3"
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
  ...
  const renderProjects = () => {
    return projects.map((project) => (
      <div key={`project-${project.id}`} className="mb-6 p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800">
        <div className="flex justify-between items-center mb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label htmlFor={`project-role-${project.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Your Role
            </label>
            <input
              type="text"
              ...
              value={project.role}
              onChange={(e) => handleChange('project', project.id, 'role', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
        ...
        <div className="mb-4">
          <label htmlFor={`project-duration-${project.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Duration
          </label>
          <input
            type="text"
            ...
            onChange={(e) => handleChange('project', project.id, 'duration', e.target.value)}
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
        ...
        <div className="mb-4">
          <label htmlFor={`project-tech-${project.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Technologies Used
          </label>
          <input
            type="text"
            ...
            onChange={(e) => handleChange('project', project.id, 'technologies', e.target.value)}
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
        ...
        <div>
          <label htmlFor={`project-description-${project.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Project Description
          </label>
          <textarea
    ...
    <div className="max-w-2xl mx-auto p-6 bg-white dark:bg-gray-900 rounded-lg shadow-lg">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={activeTab === 'hard' ? 'block' : 'hidden'}>
          <div className="mb-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">Add your technical skills like programming languages, tools, or software.</p>
          </div>
          {renderHardSkills()}
          <button
            type="button"
